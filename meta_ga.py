...
def solve_jsspt_ga(
    ...
    total_ops = sum(job_n_ops)

    # Horizon heuristic (informative only)
    max_sigma = max(max(row) for row in sigma)
    _horizon = total_proc_time + total_ops * max_sigma + 10  # not directly used
    ...
    if use_fatigue:
        ...
        Trest = 0.0
        Facc  = 0.0
        trade = Cmax     # trivial in no-fatigue mode
